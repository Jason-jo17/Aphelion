...
func test_every_mission_file_loads_without_complaint() -> void:
	assert_eq(MissionDB.load_errors.size(), 0,
		"mission files must load cleanly: " + ", ".join(MissionDB.load_errors))
	assert_gte(MissionDB.missions.size(), 12,
		"CLAUDE.md asks for at least twelve missions")
...
func test_missions_are_ordered_and_uniquely_identified() -> void:
	...
	for m in MissionDB.missions:
		...
		seen_ids[m.id] = true
		assert_false(seen_order.has(m.order),
			"missions '%s' and '%s' share order %d"
			% [m.id, seen_order.get(m.order, ""), m.order])
...
func test_every_mission_states_what_it_teaches_and_how_to_win() -> void:
	for m in MissionDB.missions:
		...
		assert_false(m.teaches.is_empty(),
			"%s does not say what it teaches — if you cannot write that line, "
			% m.id + "the mission is doing too much")
...
func test_every_mission_carries_a_reference_solution() -> void:
	for m in MissionDB.missions:
		...
		assert_false(ref.is_empty(),
			"%s has no reference solution; regenerate with "
			% m.id + "tools/refsim/author_missions.py --write")
...
func test_every_reference_solution_wins_its_mission_with_three_stars() -> void:
	for m in MissionDB.missions:
		...
		assert_eq(result.stars, 3,
			...
			+ "instructions %d/%d") % [m.id,
				result.fuel_used, m.star_fuel,
				result.elapsed, m.star_time,
				result.instruction_count, m.star_instructions])
...
func test_reference_flights_reproduce_the_recorded_numbers_exactly() -> void:
	...
	for m in MissionDB.missions:
		...
		assert_eq(result.ticks, int(ref["ticks"]),
			"%s: tick count must match the reference implementation" % m.id)
		assert_almost_eq(result.fuel_used, float(ref["fuel_used"]), 1.0e-6,
			"%s: fuel used must match the reference implementation" % m.id)
		assert_eq(result.instruction_count, int(ref["instructions"]),
			"%s: program size must match" % m.id)
		assert_eq(result.state_hash, String(ref["state_hash"]),
...
func test_an_empty_program_never_wins() -> void:
	...
	for m in MissionDB.missions:
		...
		assert_false(result.success,
			"%s is satisfied by doing nothing, which makes it not a puzzle" % m.id)
...
func _fly(mission: Mission, source: String) -> Dictionary:
	...
	assert_true(program.ok(),
		"%s: the reference solution must assemble: %s"
		% [mission.id, program.first_error_text()])
