...
func test_success_beats_failure() -> void:
	assert_true(Scoring.is_better(_result(true, 999.0, 999.0, 99),
		_result(false, 1.0, 1.0, 1)))
...
func test_anything_beats_nothing() -> void:
	assert_true(Scoring.is_better(_result(false, 0.0, 0.0, 0), null),
...
func test_the_breakdown_reports_both_numbers() -> void:
	...
	assert_almost_eq(float(rows[0]["over_fraction"]), 0.5, 1.0e-9,
		"150 against a target of 100 is 50% over")
...
func test_a_sustain_predicate_says_how_long() -> void:
	var errors: Array[String] = []
	var p := Predicate.from_dict({"sustain": 30.0,
		"of": {"sensor": "ECC", "op": "<=", "value": 0.01}}, errors)
...
func test_a_malformed_predicate_fails_loudly() -> void:
	...
	assert_eq(p.kind, Predicate.Kind.NEVER,
		"an unwinnable mission must be obvious at load, not at play")
...
func test_body_text_meets_wcag_aa_in_both_themes() -> void:
	for palette in [Tokens.DARK, Tokens.LIGHT]:
		for pair in [["text", "bg"], ["text", "surface"], ["text", "surface_high"],
				["text_muted", "bg"], ["text_muted", "surface"],
				["accent", "bg"], ["accent", "surface"], ["accent_text", "accent"],
...
func test_ui_boundaries_meet_the_three_to_one_minimum() -> void:
	for palette in [Tokens.DARK, Tokens.LIGHT]:
		for bg in ["bg", "surface", "surface_high", "overlay"]:
			assert_gte(Tokens.contrast(palette["border_strong"], palette[bg]), 3.0,
				"border_strong on %s must clear 3:1 (WCAG 1.4.11)" % bg)
		for bg2 in ["bg", "surface"]:
			assert_gte(Tokens.contrast(palette["focus"], palette[bg2]), 3.0,
				"the focus ring must be visible on %s" % bg2)
			assert_gte(Tokens.contrast(palette["text_faint"], palette[bg2]), 3.0,
...
func test_trajectory_colours_are_visible_in_both_palettes() -> void:
	for palette in [Tokens.TRAJECTORY_DEFAULT, Tokens.TRAJECTORY_COLORBLIND]:
		for role in palette:
			assert_gte(Tokens.contrast(palette[role], Tokens.DARK["bg"]), 3.0,
...
func test_every_trajectory_role_has_its_own_line_style() -> void:
	...
	for role in Tokens.TRAJECTORY_DEFAULT:
		assert_true(Tokens.TRAJECTORY_DASH.has(role),
			"'%s' needs a dash pattern as well as a colour" % role)
		var key := str(Tokens.TRAJECTORY_DASH[role])
		assert_false(seen.has(key),
			"'%s' shares a line style with '%s'" % [role, seen.get(key, "")])
		seen[key] = role


func test_the_two_palettes_cover_the_same_roles() -> void:
	for role in Tokens.TRAJECTORY_DEFAULT:
		assert_true(Tokens.TRAJECTORY_COLORBLIND.has(role),
			"the colourblind palette is missing '%s'" % role)


func test_reduced_motion_removes_every_duration() -> void:
	var was := Settings.reduced_motion
	Settings.reduced_motion = true
	assert_eq(Tokens.duration(Tokens.DUR_BASE), 0.0,
...
func test_the_mission_clock_keeps_its_width() -> void:
	assert_eq(Fmt.clock(0.0).length(), Fmt.clock(35999.0).length(),
