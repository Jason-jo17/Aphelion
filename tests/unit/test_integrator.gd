...
func test_energy_is_conserved_over_ten_orbits() -> void:
	...
	assert_lt(drift, 1.0e-11, "RK4 at 1/64 s should hold energy to far better than a part in 1e11")
...
func test_a_circular_orbit_stays_circular() -> void:
	...
	assert_almost_eq(
		float(el[Orbital.K_APOAPSIS]), r0, 0.001, "apoapsis should hold to within a millimetre"
	)
...
func test_time_is_derived_not_accumulated() -> void:
	...
	for tick in [1, 63, 64, 1000, 123456, 10000000]:
		...
		assert_eq(t * 64.0, float(tick), "tick %d should convert to a time that is exact" % tick)
...
func test_step_scale_drops_to_one_when_anything_is_happening() -> void:
	...
	assert_eq(
		_world.step_scale_for(0.0, st, burning, _profile, INF),
		1,
	...
	turning.torque = 0.5
	assert_eq(_world.step_scale_for(0.0, st, turning, _profile, INF), 1, "so must a slew")

	var low := _circular(5000.0)
	assert_eq(
		_world.step_scale_for(0.0, low, ControlInput.new(), _profile, INF),
		1,
...
func test_step_scale_never_outruns_the_flight_computer() -> void:
	...
	assert_lte(
		float(scale) * SimWorld.DT_BASE, 0.1 + 1.0e-12, "a pending WAIT must not be stepped over"
	)
...
func test_fast_forwarding_agrees_with_full_rate() -> void:
	...
	assert_lt(apart, 1.0, "three orbits of fast-forwarding should land within a metre of full rate")
...
func test_thrust_burns_propellant_at_the_rocket_equation_rate() -> void:
	...
	# Point prograde before burning. _circular() leaves the nose at angle 0,
	# which is +x — radially outward here, where the velocity is +y. Thrust in
	# that direction turns the orbit rather than speeding the ship up, and the
	# rocket equation below is about speed.
	st.angle = DetMath.PI_2
...
func test_drag_removes_energy_and_only_inside_the_atmosphere() -> void:
	...
	assert_almost_eq(
		_energy(high) / e_high, 1.0, 1.0e-11, "there is no air at 200 km, so nothing should change"
	)
...
func test_trajectory_fixtures_match_the_reference_exactly() -> void:
	...
	for case in fixtures:
		...
		while st.tick < target_tick:
			...
			if fixed_scale < 0:
				scale = world.step_scale_for(
					SimWorld.time_for_tick(st.tick), st, ctrl, profile, INF
				)
