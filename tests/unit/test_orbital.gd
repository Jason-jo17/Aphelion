...
func test_elements_match_the_reference_exactly() -> void:
	for case in _fixture:
		var el := Orbital.elements(
			...
			FixtureLoader.num(case["rx"]),
			FixtureLoader.num(case["ry"]),
			FixtureLoader.num(case["vx"]),
			FixtureLoader.num(case["vy"])
		)
		...
		for key in [
			...
			if is_inf(expected):
				assert_true(
					is_inf(got) and signf(got) == signf(expected),
...
func _element_key(fixture_key: String) -> String:
	match fixture_key:
		"sma":
			return Orbital.K_SMA
		"ecc":
			return Orbital.K_ECC
		"apoapsis":
			return Orbital.K_APOAPSIS
		"periapsis":
			return Orbital.K_PERIAPSIS
		"period":
			return Orbital.K_PERIOD
		"nu":
			return Orbital.K_TRUE_ANOMALY
		"t_apo":
			return Orbital.K_TIME_TO_APO
		"t_peri":
			return Orbital.K_TIME_TO_PERI
		"v_radial":
			return Orbital.K_RADIAL_SPEED
		"v_tangential":
			return Orbital.K_TANGENTIAL_SPEED
	return fixture_key
...
func test_escape_speed_is_root_two_times_circular() -> void:
	var r := R + 100000.0
	assert_almost_eq(Orbital.escape_speed(MU, r), Orbital.circular_speed(MU, r) * sqrt(2.0), 1.0e-9)
...
func test_time_to_apoapsis_from_periapsis_is_half_a_period() -> void:
	...
	var a := 0.5 * (rp + ra)
	var el := Orbital.elements(MU, rp, 0.0, 0.0, Orbital.vis_viva(MU, rp, a))
	assert_almost_eq(float(el[Orbital.K_TIME_TO_APO]), float(el[Orbital.K_PERIOD]) * 0.5, 1.0e-6)
...
func test_receding_hyperbola_never_reaches_periapsis_again() -> void:
	...
	var el := Orbital.elements(mu_moon, -1198571.429, -1844837.806, -279.1541, -266.7656)
	assert_true(
		is_inf(float(el[Orbital.K_TIME_TO_PERI])),
...
func test_hohmann_matches_the_two_burn_arithmetic() -> void:
	...
	var a_t := 0.5 * (r1 + r2)
	assert_almost_eq(
		float(h[0]), Orbital.vis_viva(MU, r1, a_t) - Orbital.circular_speed(MU, r1), 1.0e-9
	)
	assert_almost_eq(
		float(h[1]), Orbital.circular_speed(MU, r2) - Orbital.vis_viva(MU, r2, a_t), 1.0e-9
	)
...
func test_period_of_state_agrees_with_full_elements() -> void:
	...
	var el := Orbital.elements(MU, r, 0.0, 0.0, v)
	assert_eq(
		Orbital.period_of_state(MU, r, 0.0, 0.0, v),
		float(el[Orbital.K_PERIOD]),
