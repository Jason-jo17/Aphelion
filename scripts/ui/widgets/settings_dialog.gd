...
func _accessibility() -> Control:
	var column := _section(
		"Accessibility", "These change how the whole game is drawn, not just this screen."
	)

	var motion := CheckBox.new()
	motion.text = "Reduced motion"
	motion.tooltip_text = (
		"Transitions snap instead of easing and the ship marker "
	...
	picker.tooltip_text = (
		"A different set of hues for deuteranopia and protanopia. "
		+ "Either way every trajectory also carries a line style and a label, so "
		+ "colour is never the only signal."
	)
	picker.item_selected.connect(func(i): Settings.palette = "colorblind" if i == 1 else "default")
...
func _appearance() -> Control:
	...
	picker.item_selected.connect(func(i): Settings.theme = ["dark", "light", "system"][i])
...
func _mission_control() -> Control:
	...
	_provider_picker.item_selected.connect(
		func(i):
	...
	_model_field.text = (
		Settings.ai_model
		if not Settings.ai_model.is_empty()
		else AIProvider.default_model(Settings.ai_provider)
	)
	_model_field.tooltip_text = (
		"Editable on purpose: a model string baked into a "
	...
	_model_field.focus_exited.connect(func(): Settings.ai_model = _model_field.text.strip_edges())
	...
	_file_opt_in.tooltip_text = (
		"Anyone with access to your user directory can read "
...
func _save_key() -> void:
	...
	_key_status.add_theme_color_override(
		"font_color", Tokens.color("success" if outcome["ok"] else "danger")
	)
...
func _refresh_key_status() -> void:
	...
	if source == KeyStore.Source.NONE:
		_key_status.text = (
			"No key set. %s  Or set %s in your environment."
	...
	_key_status.text = (
		"Using %s from the %s." % [KeyStore.redact(key), KeyStore.source_name(source)]
	)
...
func _progress() -> Control:
	...
	column.add_child(
		UIKit.stat_row("Missions solved", "%d of %d" % [summary["solved"], summary["total"]])
	)
	column.add_child(UIKit.stat_row("Stars", "%d of %d" % [summary["stars"], summary["max_stars"]]))

	var reset := UIKit.button("Erase all progress", "ghost", "Delete every recorded run and draft")
	reset.add_theme_color_override("font_color", Tokens.color("danger"))
	reset.pressed.connect(
		func():
			var confirm := ConfirmationDialog.new()
			confirm.dialog_text = (
				"Erase every recorded run, personal best and draft "
